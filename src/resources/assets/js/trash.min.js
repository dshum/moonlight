...
$.blockUI();$.ajax({url:'/moonlight/trash/list',method:"POST",data:params}).done(function(response){$.unblockUI();if(response.html){$('.list-container').html(response.html)}}).fail(function(response){$.unblockUI();$.alert(response.statusText)})};var submit=function(page){$('input:hidden[name="page"]').val(page);$('form[name="trash-form"]').submit()};$('body').on('keyup change','#filter',function(){var str=$(this).val();if(str.length>0){$('ul.items > li:not(:contains("'+str+'"))').hide();$('ul.items > li:contains("'+str+'")').show()}else{$('ul.items > li').show()}});$('.search-form-links div.link').click(function(){var item=$(this).attr('item');var name=$(this).attr('name');var active=!$(this).hasClass('active');$(this).toggleClass('active');$('.search-form-params div.block[name="'+name+'"]').toggleClass('active');if(active){$('.search-form-params div.block[name="'+name+'"]').find('input, select').removeAttr('disabled')}else{$('.search-form-params div.block[name="'+name+'"]').find('input, select').attr('disabled','disabled')}
