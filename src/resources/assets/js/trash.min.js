jQuery.expr[':'].contains=function(a,i,m){return jQuery(a).text().toUpperCase().indexOf(m[3].toUpperCase())>=0};$(function(){var checked={};var getElements=function(item,addition){var params={item:item};if(addition){for(var index in addition){params[index]=addition[index]}}
$.blockUI();$('form[name="trash-form"]').ajaxSubmit({url:'/moonlight/trash/list',dataType:'json',data:params,success:function(data){$.unblockUI();if(data.html){$('.list-container').html(data.html)}},error:function(data){$.unblockUI();$.alert(data.statusText)}})};var submit=function(page){$('input:hidden[name="page"]').val(page);$('form[name="trash-form"]').submit()};$('body').on('keyup change','#filter',function(){var str=$(this).val();if(str.length>0){$('ul.items > li:not(:contains("'+str+'"))').hide();$('ul.items > li:contains("'+str+'")').show()}else{$('ul.items > li').show()}});$('.search-form-links div.link').click(function(){var item=$(this).attr('item');var name=$(this).attr('name');var active=!$(this).hasClass('active');$(this).toggleClass('active');$('.search-form-params div.block[name="'+name+'"]').toggleClass('active');if(active){$('.search-form-params div.block[name="'+name+'"]').find('input, select').removeAttr('disabled')}else{$('.search-form-params div.block[name="'+name+'"]').find('input, select').attr('disabled','disabled')}
