        ...
        <ul class="header">
                        ...
                        <ul>
                            @if (! $hasOrderProperty)
                                <li class="perpage">
                                    ...
                                    <input type="text" name="per_page" value="{{ $perPage }}">
                                </li>
                            @endif
                            @isset($columns)
                                @foreach ($columns as $column)
                                    <li name="{{ $column['name'] }}" show="{{ $column['show'] ? 'true' : 'false' }}">
            ...
            @if ($mode == 'browse' && $lastPage > 1)
                <li class="search-link">
                    <a href="{{ route('moonlight.search.item', ['item' => $currentItem->getNameId(), 'action' => 'search']) }}">Поиск</a>
                </li>
            @endif
        </ul>
        <div list>
            ...
            @if (sizeof($elements))
                ...
                <form name="save" save="true" action="{{ route('moonlight.elements.save') }}" method="POST">
                    ...
                    <table class="elements">
                        ...
                        <tr>
                            <th class="browse">
                                <span resetorder="true" title="Сортировать по умолчанию"><i class="fa fa-sort"></i></span>
                            </th>
                ...
                @if ($lastPage > 1)
                    <ul class="pager" classId="{{ isset($classId) ? $classId : ''}}" item="{{ $currentItem->getNameId() }}" page="{{ $currentPage }}" last="{{ $lastPage }}">
                        <li prev class="arrow {{ $currentPage > 1 ? 'active' : '' }}"><i class="fa fa-arrow-left"></i>
                        </li>
                        ...
                        <li next class="arrow {{ $currentPage < $lastPage ? 'active' : '' }}">
