...
@else
    <input type="hidden" name="{{ $name }}" data-property="{{ $name }}" value="{{ $value ? $value->format('H:i:s') : '' }}" class="time">
    @if ($value)
        <div>
            <label>{{ $title }}:</label> <span class="datetime-container" data-property="{{ $name }}">
                <span class="timepicker" data-property="{{ $name }}">{{ $value->format('H:i:s') }}</span>
            </span> <span data-name="{{ $name }}" class="error"></span>
        </div>
    @else
        <div>
            <label>{{ $title }}:</label> <span class="datetime-container" data-property="{{ $name }}">
            <span class="timepicker" data-property="{{ $name }}">Не определено</span>
        </span> <span data-name="{{ $name }}" class="error"></span>
        </div>
    @endif
        ...
        <div class="block">
            ...
            <table class="hours">
                @for ($i = 0; $i < 6; $i++)
                    <tr>
                        @for ($j = 0; $j < 4; $j++)
                            <td data-value="{{ sprintf('%02d', $i * 4 + $j) }}" class="{{ $value && $value->format('H') == $i * 4 + $j ? 'active' : '' }}">{{ sprintf('%02d', $i * 4 + $j) }}</td>
                        @endfor
                    </tr>
                @endfor
            </table>
